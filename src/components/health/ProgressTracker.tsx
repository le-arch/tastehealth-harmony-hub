...
const ProgressTracker = () => {
    ...
    const sum = progressData.reduce(
      (acc, curr) => ({
        water_goal_percentage:
          (acc.water_goal_percentage || 0) + (curr.water_goal_percentage || 0),
        sleep_goal_percentage:
          (acc.sleep_goal_percentage || 0) + (curr.sleep_goal_percentage || 0),
        calories_goal_percentage:
          (acc.calories_goal_percentage || 0) +
          (curr.calories_goal_percentage || 0),
        exercise_goal_percentage:
          (acc.exercise_goal_percentage || 0) +
          (curr.exercise_goal_percentage || 0),
        overall_progress_percentage:
          (acc.overall_progress_percentage || 0) +
          (curr.overall_progress_percentage || 0),
      }),
      {
  ...
  const pieData = latest
    ? [
        {
          name: t.water,
          value: latest.water_goal_percentage || 0,
          color: COLORS.water,
        },
        {
          name: t.sleep,
          value: latest.sleep_goal_percentage || 0,
          color: COLORS.sleep,
        },
        {
          name: t.calories,
          value: latest.calories_goal_percentage || 0,
          color: COLORS.calories,
        },
        {
          name: t.exercise,
          value: latest.exercise_goal_percentage || 0,
      ...
      <CardContent>
        ...
        >
          ...
          <TabsContent value="overview" className="space-y-4 gap-4 ">
            ...
            <br />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-4 space-y-4">
              {[
                {
                  label: t.water,
                  key: "water_goal_percentage",
                  color: COLORS.water,
                  icon: <Droplet />,
                },
                {
                  label: t.sleep,
                  key: "sleep_goal_percentage",
                  color: COLORS.sleep,
                  icon: <Moon />,
                },
                {
                  label: t.calories,
                  key: "calories_goal_percentage",
                  color: COLORS.calories,
                  icon: <Utensils />,
                },
                {
                  label: t.exercise,
                  key: "exercise_goal_percentage",
                  color: COLORS.exercise,
                  icon: <Activity />,
                },
              ].map((item, idx) => (
                  ...
                  {latest ? (
                    <>
                      <div className="text-2xl font-bold">
                        {(
                          latest[item.key as keyof DailyProgress] as number
                        )?.toFixed(1) || 0}
                        %
                      </div>
                      {averages && (
                        <div className="text-sm text-gray-500">
                          {t.weeklyAverage}:{" "}
                          {averages[
                            item.key.split("_")[0] as keyof typeof averages
                          ].toFixed(1)}
                          %
                        </div>
                      )}
