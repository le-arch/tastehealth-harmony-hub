...
import { CalendarCheck, Flame, Moon, Dumbbell, Droplet, TrendingUp, Minus, Activity, Heart, AlertTriangle, CheckCircle2, ArrowUpRight, ArrowDownRight } from "lucide-react";
...
const WeeklySummary: React.FC = () => {
  ...
  const goals = useMemo(() => {
    try {
      ...
      return { calories: parseInt(prefs.calorieGoal) || 2000, sleep: 8, exercise: 150, water: 56 };
  ...
  useEffect(() => {
    ...
    if (dayOfWeek === 0 && lastNotified !== weekKey && (totalCal > 0 || totalSleep > 0)) {
      const notificationDate = format(new Date(), 'MMMM d, yyyy');
      const existing = JSON.parse(localStorage.getItem(LS_KEYS.NOTIFICATIONS) || '[]');
      existing.unshift({
        id: crypto.randomUUID(),
        date: new Date().toISOString(),
        title: `📊 Weekly Summary — ${notificationDate}`,
        message: `Health Score: ${healthScore}/100. Avg ${avgCal} cal/day, ${avgSleep} hrs sleep, ${totalExercise} min exercise, ${totalWater} cups water. Week of ${notificationDate}.`,
        read: false,
      });
                ...
                {stats.map((s, idx) => (
                  <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <TrendIndicator current={s.curr} previous={s.prev} />
                    </div>
